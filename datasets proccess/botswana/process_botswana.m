...
for l = 1:floor(L/patch_size)
    for w = 1:floor(W/patch_size)
        ...
        % Crating RGB image
        rgb = Botswana_rgb((l-1)*patch_size +1: l*patch_size, (w-1)*patch_size +1: w*patch_size,:);
        
        % Saving ref, y, pan, RGB
        save(strcat(file_name, '/botswana4_', num2str(count, '%02d'),'.mat'), 'ref', 'y', 'pan','rgb');
        count=count+1;
    end
end